---
title: "BioMove swallow-parasite manuscript" ## name of your project and analysis step
description: "cleaning capture-mark-recapture scripts"
author:
    - name: "Marius Grabow"
...
clean_cmr_data<-function(data){
  ...
  group_by(ring_id)%>%
    #filling body measurements from the same individual when they were missing during a recapture
  fill(tars_mm,
       wing_mm,
       X8prim_mm,
       fat,
...
# read single data frames and apply function for cleaning the data frames
...
cmr_filter<-cmr_all%>%
  ...
  #filter just both swallow species (there are some sparrows, etc.)
  ...
  group_by(ring_id)%>%
  # manually replace recapture (y/n) if an animal was captured before
  mutate(ring_recap=replace(ring_recap, row_number() > 1, "y"))%>%
  mutate(ring_recap=ifelse(row_number()==1,"n",ring_recap))%>%
  ungroup()%>%
  ...
  mutate(c_2019=if_else(year==2019,1,0),
         ...
         c_2023=if_else(year==2023 & date=="2023-05-23" | date=="2023-05-24",1,0),
         c_2023=na_if(c_2023,0),
         r_2023=if_else(year==2023 & date=="2023-06-06" | date=="2023-06-07",1,0),
         r_2023=na_if(r_2023,0)
         )%>%
  fill(c_2019,c_2020,r_2020,c_2021,r_2021,c_2022,r_2022,c_2023,r_2023,.direction = "updown")%>%
  ungroup()%>%
  mutate(c_2019=replace_na(c_2019,0),
         ...
         c_2023=replace_na(c_2023,0),
         r_2023=replace_na(r_2023,0),
  ...
  mutate(type=case_when(
    ...
    date == "2023-05-23"|
    date == "2023-05-24"~ "capture_2023",
    date == "2023-06-06"|
    date == "2023-06-07"~ "recapture_2023",
    date == "2023-06-26"|
    date == "2023-06-27"|
    date == "2023-06-28"~ "Wiebke"
      ))%>%
    mutate(capture_type=case_when(
    year == 2019 ~ "cmr",
    date == "2020-06-06"|
    date == "2020-06-07"~ "cmr",
    date == "2020-06-16"|
  ...
  fill(tars_mm,
       ...
       age,
       date)%>%
  # minimal age of indiviudal
  mutate(min_age=if_else(age=="a" & min_age < 50,min_age+365,min_age))%>%
  ungroup()%>%
  # remove uncertainty in sex assignment
    mutate(sex=case_when(
    sex == "?" | is.na(sex) ~ "unknown",
    sex == "f" | sex == "f?" ~ "female",
    sex == "m" | sex == "m?" ~ "male"
  ))%>%
  # this tag did not work
  mutate(tag_id=ifelse(location == "MVA-Kutzerow" & tag_id=="2887",NA,tag_id))
